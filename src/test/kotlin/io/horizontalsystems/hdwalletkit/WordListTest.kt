...
class WordListTest {
    ...
    fun wordList_eachLanguage_firstAndLastWordAccessible() {
        for (language in Language.values()) {
            ...
            assertNotNull("wordList($language) returned null", wordList)
            // BIP39: 2048 words per language, indices 0..2047
            val firstWord = wordList[0]
            val lastWord = wordList[2047]
            assertTrue("$language first word should not be blank", firstWord.isNotBlank())
            assertTrue("$language last word should not be blank", lastWord.isNotBlank())
        }
    }
    ...
    fun wordListStrict_eachLanguage_loadsSuccessfully() {
        for (language in Language.values()) {
            ...
            assertNotNull("wordListStrict($language) returned null", wordList)
        }
    }
