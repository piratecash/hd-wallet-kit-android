...
class Mnemonic {
    ...
    fun generate(
        ...
        return toMnemonic(PlatformCrypto.randomBytes(strength.entropyLength / 8), language)
    }
    ...
    fun toMnemonic(entropy: ByteArray, language: Language): List<String> {
        ...
        val concatBits = BooleanArray(entropyBits.size + checksumLengthBits).also { bits ->
            entropyBits.copyInto(bits)
            hashBits.copyInto(bits, destinationOffset = entropyBits.size, endIndex = checksumLengthBits)
        }
    ...
    fun toSeed(mnemonicKeys: List<String>, passphrase: String = ""): ByteArray {
        ...
        val pass = PlatformCrypto.normalizeNfkd(mnemonicKeys.joinToString(separator = separator))
        val salt = PlatformCrypto.normalizeNfkd("mnemonic$passphrase")

        return pbkdf2HmacSha512(pass.encodeToByteArray(), salt.encodeToByteArray(), PBKDF2_ROUNDS, 64)
    }
    ...
    fun toEntropy(mnemonicKeys: List<String>, wordList: MnemonicWordList): ByteArray {
        ...
        entropy.sha256()[0].toBits().let { hashFirstByteBits ->
    ...
    private fun hash(input: ByteArray, offset: Int, length: Int): ByteArray {
        return input.copyOfRange(offset, offset + length).sha256()
    }
